...
| `gsea_barplot` | **Row order FIXED, not accepted.** `gs_plot_bar()` sets the y-axis factor's levels in ascending-`stat` order but does not reorder the rows themselves, so `gs_source` -- the table `gs_save()` writes beside the figure -- listed pathways in selection order instead of the old ascending-NES order. The shim now re-sorts `$data` and `gs_source`. Verified nothing rendered moved: bar position is `rank(stat)` ascending both before and after, and the row order now matches the old golden exactly (`-4.135 ... 4.078`). Remaining: one extra zero-line layer (`R/gs-plot-bar.R:84`) and `linewidth` 0.545 -> 0.4, both `gs_plot_bar()`'s own design. | measured |
